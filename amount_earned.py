...
class display(tk.Frame):
    def __init__(self):
        ...
        self.title_text = tk.Label(self.title_frame, text="Amount Earned by Products", pady=30, font=("Lucida Grande", 20, 'bold'))
    ...
    def navigation_frame(self):
        ...
        self.columns = ["ID", "Name","Price","Amount_Earned"]
    ...
    def display_frame(self):
        ...
        self.label_price.grid(row=2, column=0, sticky=tk.NW, padx=5)
        self.entry_price = tk.Entry(self.left_frame, width=35,justify='left')
        self.entry_price.grid(row=2, column=1)

        self.label_amount_earned = tk.Label(self.left_frame,text="Amount Earned: ", anchor=tk.W,justify='left')
        self.label_amount_earned.grid(row=3, column=0, sticky=tk.NW, padx=5)
        self.entry_amount_earned = tk.Entry(self.left_frame, width=35,justify='left')
        self.entry_amount_earned.grid(row=3, column=1)

    def table_frame(self):
        self.fields = ["Id", "Name","Price", "Amount Earned"]
        ...
        self.product_table.column(self.fields[1], width=300)
    ...
    def focus_data(self, event):
        ...
        self.entry_amount_earned.insert('end', product_data[3])

    def load_data(self):
        data = database.select('''SELECT p.product_id as "ID", p.name as "Name", p.price as "Price", p.price * SUM(od.quantity) + "php" as "Amount_Earned"
